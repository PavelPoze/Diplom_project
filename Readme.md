...
## Инcтрукция по работе с постман
...
Т.к. подтверждение того, что проверка прошла успешно именно код 200. После этого в поле Test появится некая функция на языке Java.

![Test]((58).png)

Расшифруем первую строку:

1. pm - это сам постман

2. test - это объект в постман, в нашем случае сам тест, который проверяет появится ли код 200, или нет.
...
Для запуска теста нужно нажать кнопку синию SEND. После выполнения открыть вкладку Test rezults и мы видим результат прохождения теста наш код 200 со значением PASS, значит всё прошло удачно.

![Code_200]((59).png)


Для далнейших действий нам нужно добавить еще один код с помощью сниппеда 

        Responce body: JSON value chek

который находится в том же меню сниппедов
